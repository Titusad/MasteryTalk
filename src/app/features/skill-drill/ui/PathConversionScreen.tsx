 ...
 *
 *  Copy anchors v9.0:
 *  - "$24.99 one-time — 12 sessions, permanent access"
 *  - "$2.08/session — less than a coffee"
 *  - "All 5 paths for $59.99 (save 52%)"
 *
 *  Props:
...
const SCENARIO_COPY: Record<string, ScenarioCopy> = {
  interview: {
    ...
    valueCards: [
      {
        ...
        desc: "Each level raises the difficulty — from behavioral basics to high-pressure technical rounds with adaptive AI.",
      },
      {
        icon: "star",
        title: "12 unique sessions, zero repetition",
    ...
    conversionHook: "At $2.08/session, it costs less than a coffee. 12 sessions to interview mastery.",
  },
  sales: {
    ...
    valueCards: [
      {
        icon: "trending",
        title: "4 levels of increasing pressure",
        desc: "From discovery calls to C-suite boardrooms — the AI escalates objection difficulty based on your actual performance.",
      },
      {
        icon: "star",
        title: "12 unique negotiation scenarios",
    ...
    conversionHook: "One closed deal covers years of practice. 12 sessions for $24.99 — invest in your edge.",
  },
  networking: {
    ...
    conversionHook: "One connection can change your career. 12 preparation sessions for $24.99.",
  },
};
...
export function PathConversionScreen({
  ...
  const product = PATH_PRODUCTS.single_path;
  const bundle = PATH_PRODUCTS.all_access;
    ...
    <div className="w-full min-h-[calc(100dvh-4rem)] bg-[#0f172b] flex flex-col items-center justify-center px-4 py-8">
      ...
      >
        ...
        >
          ...
          Get the Full Path — ${product.price}
          <ArrowRight className="w-4 h-4" />
        </button>

        {/* Secondary: All-Access Bundle */}
        <button
          onClick={onPurchasePath}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-lg bg-[#1e293b] border border-[#334155] text-[#cbd5e1] text-sm font-medium hover:bg-[#334155] transition-colors cursor-pointer"
        >
          <Layers className="w-4 h-4 text-[#22c55e]" />
          All 5 Paths — ${bundle.price}
          <span className="text-[10px] bg-[#22c55e]/15 text-[#22c55e] px-1.5 py-0.5 rounded font-medium ml-1">
            SAVE 52%
          </span>
        </button>
